...
class TestRawLineBuilder:
    ...
    def test_simple_assignment(self, tmp_path):
        ...
        result = RawLineBuilder.read_file_to_raw_lines(file)
        assert len(result) == 1
        assert result[0].indent == 0
        assert result[0].abs_indent == 0
    ...
    def test_multiple_dedent(self, tmp_path):
        ...
        abs_indents = [line.abs_indent for line in result]
        assert levels == [0, 1, 2, 1, 2]
        assert abs_indents == [0, 4, 8, 4, 8]
    ...
    def test_empty_lines_preserved(self, tmp_path):
        ...
        assert [line.abs_indent for line in result] == [0, 0, 4]
...
'''
    ...
    def test_syntax_error_raises(self, tmp_path):
        ...
        with pytest.raises(SyntaxError):
            RawLineBuilder.read_file_to_raw_lines(file)
...
'''
        ...
        abs_indents = [line.abs_indent for line in result if not line.is_string_content]
        assert levels == [0, 1, 1, 1, 1]
        assert abs_indents == [0, 4, 4, 4, 4]

        string_line = result[3]
        assert string_line.is_string_content is True
        assert string_line.indent == 1
        assert string_line.abs_indent == 8

    # Тесты для strip_comments=True
    def test_strip_comments_removes_comment_tokens(self, tmp_path):
        code = "x = 1  # comment\n"
        file = write_temp_file(tmp_path, code)
        result = RawLineBuilder.read_file_to_raw_lines(file, strip_comments=True)
        assert len(result) == 1
        assert result[0].raw_strs == ["x", "=", "1"]
    ...
    # Дополнительные тесты для abs_indent в нестандартных ситуациях
    def test_mixed_spaces_and_tabs_indent(self, tmp_path):
        code = "def foo():\n\t  pass\n"
        file = write_temp_file(tmp_path, code)
        try:
            result = RawLineBuilder.read_file_to_raw_lines(file)
            assert result[1].abs_indent == 3

        except SyntaxError:
            pass

    def test_abs_indent_empty_line(self, tmp_path):
        code = "def foo():\n    \n    pass\n"
        file = write_temp_file(tmp_path, code)
        result = RawLineBuilder.read_file_to_raw_lines(file)
        assert result[1].raw_strs == []
